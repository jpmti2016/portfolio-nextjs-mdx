export default function Footer() {
  return (
    <footer
      id="contact"
      //   className="py-8 sm:py-20"
      className="py-8 text-gray-900 bg-gray-100 sm:py-20 dark:bg-gray-900 dark:text-gray-200"
    >
      <div className="">
        {/* <div className="text-yellow-600 dark:bg-blue-900"> */}
        <div className="flex flex-col items-center content-center justify-center text-xl">
          <div className="flex mb-3 space-x-2 text-2xl sm:text-3xl sm:space-x-4">
            <a
              className="block"
              aria-label="personal tweeter account"
              href="https://twitter.com/jpmti2016"
              target="_blank"
              rel="noreferrer noopener"
            >
              <svg
                className="w-8 h-8 rounded-full fill-current dark:text-gray-200 ring-gray-600 dark:ring-gray-200 ring-2 sm:h-10 sm:w-10 hover:ring-gray-600 hover:bg-gray-600 hover:text-blue-100 dark:hover:text-gray-900"
                clipRule="evenodd"
                fillRule="evenodd"
                strokeLinejoin="round"
                strokeMiterlimit="2"
                viewBox="-89.009 -46.884 643.937 446.884"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M154.729 400c185.669 0 287.205-153.876 287.205-287.312 0-4.37-.089-8.72-.286-13.052A205.304 205.304 0 00492 47.346c-18.087 8.044-37.55 13.458-57.968 15.899 20.841-12.501 36.84-32.278 44.389-55.852a202.42 202.42 0 01-64.098 24.511C395.903 12.276 369.679 0 340.641 0c-55.744 0-100.948 45.222-100.948 100.965 0 7.925.887 15.631 2.619 23.025-83.895-4.223-158.287-44.405-208.074-105.504A100.739 100.739 0 0020.57 69.24c0 35.034 17.82 65.961 44.92 84.055a100.172 100.172 0 01-45.716-12.63c-.015.424-.015.837-.015 1.29 0 48.903 34.794 89.734 80.982 98.986a101.036 101.036 0 01-26.617 3.553c-6.493 0-12.821-.639-18.971-1.82 12.851 40.122 50.115 69.319 94.296 70.135-34.549 27.089-78.07 43.224-125.371 43.224A204.9 204.9 0 010 354.634c44.674 28.645 97.72 45.359 154.734 45.359"
                  fillRule="nonzero"
                />
              </svg>
            </a>
            <a
              className="block"
              aria-label="personal linkedin account"
              href="https://www.linkedin.com/in/yampier-medina-423641169"
              target="_blank"
              rel="noreferrer noopener"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="2500"
                height="2500"
                viewBox="7.025 7.025 497.951 497.95"
                className="w-8 h-8 text-gray-200 bg-gray-600 border-gray-600 rounded-full fill-current dark:bg-gray-200 ring-gray-600 dark:ring-gray-200 ring-2 dark:text-gray-900 hover:text-gray-600 hover:bg-gray-200 dark:hover:text-gray-600 dark:hover:bg-gray-900 sm:h-10 sm:w-10"
              >
                <linearGradient
                  id="a"
                  gradientUnits="userSpaceOnUse"
                  x1="-974.482"
                  y1="1306.773"
                  x2="-622.378"
                  y2="1658.877"
                  gradientTransform="translate(1054.43 -1226.825)"
                >
                  <stop offset="0" stopColor="#2489be" />
                  <stop offset="1" stopColor="#0575b3" />
                </linearGradient>
                <path
                  className=""
                  d="M256 7.025C118.494 7.025 7.025 118.494 7.025 256S118.494 504.975 256 504.975 504.976 393.506 504.976 256C504.975 118.494 393.504 7.025 256 7.025zm-66.427 369.343h-54.665V199.761h54.665v176.607zM161.98 176.633c-17.853 0-32.326-14.591-32.326-32.587 0-17.998 14.475-32.588 32.326-32.588s32.324 14.59 32.324 32.588c.001 17.997-14.472 32.587-32.324 32.587zm232.45 199.735h-54.4v-92.704c0-25.426-9.658-39.619-29.763-39.619-21.881 0-33.312 14.782-33.312 39.619v92.704h-52.43V199.761h52.43v23.786s15.771-29.173 53.219-29.173c37.449 0 64.257 22.866 64.257 70.169l-.001 111.825z"
                />
              </svg>
            </a>
            <a
              className="block"
              aria-label="personal email account"
              href="mailto:jpmti2016@gmail.com"
              target="_blank"
              rel="noreferrer noopener"
            >
              <svg
                className="w-8 h-8 text-gray-200 bg-gray-600 rounded-full fill-current dark:bg-gray-200 sm:h-10 sm:w-10 ring-2 ring-gray-600 dark:ring-gray-200 dark:text-gray-900 hover:text-gray-600 hover:bg-gray-200 dark:hover:text-gray-600 dark:hover:bg-gray-900"
                clipRule="evenodd"
                fillRule="evenodd"
                imageRendering="optimizeQuality"
                shapeRendering="geometricPrecision"
                textRendering="geometricPrecision"
                viewBox="0 0 3000 3000"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M1500 0c828 0 1500 672 1500 1500s-672 1500-1500 1500S0 2328 0 1500 672 0 1500 0zm751 902l-1456-4 728 578zm-900 735l-636-500v950h1573v-930l-590 482c-200 163-140 160-347-2zM723 716h1569c127 0 230 104 230 230v1100c0 127-104 230-230 230H723c-127 0-230-104-230-230V946c0-127 104-230 230-230z"
                  fill="currentColor"
                />
              </svg>
            </a>
            <a
              className="block"
              aria-label="personal github account"
              href="https://github.com/jpmti2016"
              target="_blank"
              rel="noreferrer noopener"
            >
              <svg
                className="w-8 h-8 rounded-full fill-current dark:text-gray-200 ring-gray-600 dark:ring-gray-200 ring-2 sm:h-10 sm:w-10 hover:ring-gray-600 hover:bg-gray-600 hover:text-gray-200 dark:hover:text-gray-900"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 999.937 999.937"
              >
                <path d="M0 499.968c0-138.012 48.825-255.843 146.476-353.493C244.125 48.825 361.956 0 499.969 0 637.98 0 755.811 48.825 853.462 146.475c97.649 97.65 146.475 215.481 146.475 353.493s-48.825 255.843-146.475 353.493c-97.65 97.65-215.481 146.476-353.493 146.476-138.013 0-255.844-48.825-353.493-146.476C48.825 755.812 0 637.979 0 499.968zm54.684 0c0 122.389 43.617 227.199 130.851 314.434 87.234 87.233 192.045 130.851 314.434 130.851 122.388 0 227.199-43.617 314.433-130.851 87.234-87.234 130.851-192.045 130.851-314.434 0-122.388-43.616-227.199-130.851-314.433C727.168 98.301 622.356 54.684 499.969 54.684c-122.389 0-227.199 43.617-314.434 130.851C98.301 272.769 54.684 377.58 54.684 499.968zm140.617 107.415c-2.604-2.604-2.604-5.208 0-7.812 5.207-5.208 11.718-7.161 19.529-5.859 7.812 1.302 12.369 2.604 13.671 3.906 9.114 3.906 19.205 12.694 30.271 26.366 11.066 13.671 19.855 23.11 26.366 28.318 31.248 26.04 63.146 27.993 95.696 5.859 2.604-9.114 6.836-16.926 12.694-23.437 5.858-6.51 11.393-11.066 16.601-13.671 5.209-2.604 14.323-6.51 27.343-11.718-42.967-3.906-77.795-11.393-104.486-22.46s-47.849-25.063-63.473-41.989c-20.832-23.436-33.527-54.033-38.084-91.791-4.558-37.758-1.628-72.261 8.789-103.509 7.812-19.53 18.879-37.106 33.2-52.731-10.416-32.55-8.463-69.657 5.859-111.321 41.664 2.604 79.422 16.926 113.274 42.966 65.1-16.926 132.804-17.577 203.111-1.953 9.114-6.51 23.111-14.647 41.989-24.413 18.879-9.765 42.641-15.299 71.285-16.601 5.208 14.322 8.789 31.248 10.742 50.778s.977 37.758-2.929 54.684c29.945 31.248 45.569 72.912 46.871 124.992 0 41.664-7.16 76.167-21.482 103.509s-39.711 50.127-76.167 68.354c-24.738 11.719-57.288 18.879-97.65 21.483 18.229 9.114 31.574 18.554 40.037 28.319 8.464 9.765 13.997 25.063 16.602 45.895v61.521l1.952 59.566c3.906 6.511 8.464 12.044 13.672 16.602 5.208 4.557 9.765 7.812 13.671 9.765 3.905 1.953 5.208 4.883 3.905 8.789-1.302 3.906-6.51 5.859-15.623 5.859-22.135 0-39.711-7.812-52.731-23.437-3.906-6.51-5.859-14.321-5.859-23.436v-93.744c0-10.416-2.604-17.903-7.812-22.46-5.208-4.558-10.416-7.487-15.624-8.789v123.039c0 22.134 2.604 36.456 7.812 42.966s8.463 13.021 9.766 19.53c1.302 1.302.325 2.278-2.93 2.929-3.254.65-8.788-.325-16.601-2.929-16.926-3.906-28.644-12.695-35.154-26.366-6.51-13.671-9.765-28.319-9.765-43.943V667.926H488.25v121.086c0 15.624-3.256 30.598-9.766 44.919-9.113 18.229-26.04 27.993-50.777 29.295-3.906-1.302-5.859-2.604-5.859-3.905 1.302-1.303 5.208-7.812 11.718-19.53 1.302-2.604 2.93-7.812 4.883-15.624 1.954-7.812 2.93-16.926 2.93-27.342v-123.04c-5.208 1.302-10.091 4.231-14.648 8.789s-6.836 12.044-6.836 22.46v93.744c0 9.113-1.953 16.926-5.859 23.436-11.718 15.624-29.295 23.437-52.73 23.437-9.114 0-14.322-1.953-15.624-5.859-1.303-2.604-.977-4.883.977-6.836s4.883-4.232 8.789-6.836c3.906-2.604 6.51-4.558 7.812-5.859 5.208-3.906 9.113-9.114 11.718-15.624 3.906-5.208 4.882-18.879 2.929-41.013s-2.279-36.456-.977-42.966c-33.853 11.718-68.355 5.858-103.51-17.577-10.416-10.416-20.832-25.39-31.248-44.919-7.812-14.323-23.436-31.249-46.871-50.779z" />
              </svg>
            </a>
          </div>
          <p className="text-base font-playfair">Created by Yampier Medina</p>
          <p className="text-sm font-code">© 2020 MIT Licence</p>
        </div>
      </div>
    </footer>
  );
}
