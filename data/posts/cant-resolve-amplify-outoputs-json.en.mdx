...
- Update the project (app) hosting Build settings to add a build command `- npx ampx pipeline-deploy --branch $AWS_BRANCH --app-id $AWS_APP_ID`:
...
- Attach the `AdministratorAccess-Amplify` policy under AWS managed policies to the amplify-policy for the the user created in the IAM Identity Center like explained in https://docs.amplify.aws/react/start/account-setup/
